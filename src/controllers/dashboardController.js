  ...
  try {
    ...
    if (instIdStr) {
      institute = await Institute.findById(instIdStr)
        .select(
          "name status subscriptionPlan subscriptionEnd adminUser tuitionType quizFeatureEnabled recordedLecturesFeatureEnabled releaseVideosFeatureEnabled brandingEnabled themeColor logoUrl allowedFeatures whatsappSettings"
        );
    }
